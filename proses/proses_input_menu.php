    ...
    if ($statusUpload == 0) {
        ...
        if (mysqli_num_rows($select) > 0) {
            ...
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "INSERT INTO tabel_daftar_menu (foto_menu,nama_menu,keterangan,kategori,harga,stok) values ('" . $kode_random . $_FILES['foto']['name'] . "','$nama_menu', '$keterangan', '$kategori', '$harga', '$stok')");
                if ($query) {
