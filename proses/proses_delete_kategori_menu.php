...
if (!empty($_POST['delete_kategori_validate'])) {
    $select = mysqli_query($conn, "SELECT kategori FROM tabel_daftar_menu WHERE kategori = '$id'");
    if (mysqli_num_rows($select) > 0) {
        ...
        $query = mysqli_query($conn, "DELETE FROM tabel_kategori_menu WHERE id_kategori_menu = '$id'");
        if ($query) {
