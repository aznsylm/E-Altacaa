...
if (!empty($_POST['edit_kategori_menu_validate'])) {
    $select = mysqli_query($conn, "SELECT kategori_menu FROM tabel_kategori_menu WHERE kategori_menu = '$katmenu'");
    if (mysqli_num_rows($select) > 0) {
        $massage = '<script>alert("Kategori Menu Sudah Ada");
                    window.location="../katmenu"</script>';
    } else {
        $query = mysqli_query($conn, "UPDATE tabel_kategori_menu SET jenis_menu='$jenismenu',kategori_menu='$katmenu' WHERE id_kategori_menu='$id'");
        if ($query) {
