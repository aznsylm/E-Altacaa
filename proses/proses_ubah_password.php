...
if (!empty($_POST['ubah_password_validate'])) {
    $query = mysqli_query($conn, "SELECT * FROM tabel_user WHERE username = '$_SESSION[username_altaca]' && password = '$passwordlama' ");
    $hasil = mysqli_fetch_array($query);
    if ($hasil) {
        if ($passwordbaru == $konfirmasipassword) {
            $query = mysqli_query($conn, "UPDATE tabel_user SET password='$passwordbaru' WHERE username = '$_SESSION[username_altaca]'");
            if ($query) {
