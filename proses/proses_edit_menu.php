...
if (!empty($_POST['edit_menu_validate'])) {
    ...
    if ($statusUpload == 0) {
        ...
        $select = mysqli_query($conn, "SELECT * FROM tabel_daftar_menu WHERE nama_menu = '$nama_menu'");
        if (mysqli_num_rows($select) > 0) {
            $massage = '<script>alert("Nama Menu yang Dimasukkan Telah Ada");
                    window.location="../minuman"</script>';
        } else {
            if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
                $query = mysqli_query($conn, "UPDATE tabel_daftar_menu SET foto_menu='" . $kode_random . $_FILES['foto']['name'] . "', nama_menu='$nama_menu', keterangan='$keterangan', kategori='$kategori', harga='$harga', stok='$stok' WHERE id='$id'");
                if ($query) {
